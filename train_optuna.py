...
models = {"ALS": AlternatingLeastSquares, "LightFM": LightFM}
...
def opt_metric(trial, dataset, valid_user_ids, top_k, metric, model):
    if model == "ALS":
        ...
        rec_model = ImplicitALSWrapperModel(
            models[model](**params), fit_features_together=True
        )
        ...
        params = {
            ...
            "no_components": trial.suggest_categorical("no_components", [32, 64, 100]),
            "learning_rate": trial.suggest_loguniform("learning_rate", 1e-5, 0.99),
            "user_alpha": trial.suggest_loguniform("user_alpha", 1e-5, 0.99),
            "item_alpha": trial.suggest_loguniform("item_alpha", 1e-5, 0.99),
        }
        rec_model = LightFMWrapperModel(
            models[model](**params), epochs=N_EPOCHS, num_threads=NUM_THREADS
        )
